...
export const CashManagementModal: React.FC<CashManagementModalProps> = ({ isOpen, onClose }) => {
  ...
  const executeMovement = (managerName: string) => {
    setIsVerifying(false);
    ...
    <>
      ...
      {isVerifying && (
        <ManagerOverrideModal 
          actionName={type === 'PAY_IN' ? 'Authorize Pay In' : 'Authorize Pay Out'}
          onCancel={() => setIsVerifying(false)}
          onSuccess={executeMovement}
        />
      )}

      <div className="fixed inset-0 z-40 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-fadeIn">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col transition-all">
          ...
          {successData ? (
            ...
            <>
              ...
              <form onSubmit={handleInitiate} className="p-6 space-y-6">
